...
import com.spirit.koil.api.design.particle.VanillaParticleSprites.SpriteSet;
 ...
 */
public final class UiParticleAuthoringExample {
    private UiParticleAuthoringExample() {
    }

    public static void registerExample() {
        UiParticleRegistry.register(
                UiParticleEffect.builder("example_diamond_sparkle")
                        .themeColor(0x55FFFF)
                        .onBegin(ctx -> {
                            ctx.pulse(0.0F, 0.35F, 0x55FFFF, 0.8F);
                            for (int i = 0; i < 18; i++) {
                                spawnDiamondSpark(ctx, i % 8 == 0
                                        ? UiParticleEngine.Layer.FRONT
                                        : UiParticleEngine.Layer.BACK);
                            }
                        })
                        .onTick((ctx, dt) -> {
                            int count = ctx.emissionCount("diamond", 4.0F, dt);
                            for (int i = 0; i < count; i++) {
                                spawnDiamondSpark(ctx, UiParticleEngine.Layer.BACK);
                            }
                        })
                        .build()
        );
    ...
    private static void spawnDiamondSpark(
            UiParticleEngine.EffectContext ctx,
            UiParticleEngine.Layer layer
    ) {
        double angle = ctx.random(0.0F, (float) (Math.PI * 2.0));
        float speed = ctx.random(40.0F, 95.0F);

        ctx.spawn(ctx.particle(UiParticleEngine.Shape.CRIT)
