...
public final class RichChatPrivateMessageBridge {
    ...
    private static String[] collapseWrappedLongToken(String[] bodyLines) {
        ...
        // Attachment markers are one protected row per line. Joining them as
        // an ordinary long token removes the vertical space that the media
        // renderer needs, leaving a full image/video painted over later chat
        // rows. The same rule protects multiline code/LaTeX render markers.
        for (String line : bodyLines) {
            if (LocalRichAttachmentBridge.containsMarker(line)
                    || RichChatCodeBlockBridge.containsMarker(line)
                    || RichChatLatexTextureCache.containsMarker(line)) {
                return bodyLines;
            }
        }
    ...
    /**
     * Width native ChatHud must add back while splitting tagged PM rows.
     * These control prefixes are removed by Koil's renderer, so allowing them
     * to consume visible width creates a large false gap at the row's right.
     */
    public static int nativeWrapWidthAdjustment(TextRenderer renderer, String visible) {
        if (renderer == null || visible == null || visible.isEmpty()) {
            return 0;
        }
        int widest = 0;
        for (String line : splitLines(visible)) {
            widest = Math.max(widest, renderer.getWidth(leadingMarkerPrefix(line)));
        }
        return widest;
    }
