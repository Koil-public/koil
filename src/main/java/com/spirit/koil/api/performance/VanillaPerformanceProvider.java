...
final class VanillaPerformanceProvider implements PerformanceOptimizationProvider {
    ...
    public List<PerformanceSettingDescriptor> settings(MinecraftClient client, PerformanceSnapshot snapshot) {
        ...
        settings.add(setting("entity_distance", "Entity distance", "entity/render", "current", "0.75", "safe", "Reduces distant entity rendering pressure."));
        settings.add(setting("max_fps", "Max FPS", "frame pacing", "current", snapshot.worldType().equals("server") ? "120" : "144", "optional", "Caps frame output for steadier frame pacing."));
        settings.add(setting("clouds", "Clouds", "render", "current", "off", "optional", "Removes optional cloud rendering cost."));
        settings.add(setting("mipmaps", "Mipmaps", "memory/vram", "current", "one step lower", snapshot.memoryPressure() > 0.80D ? "caution" : "optional", "Lowers texture memory and reload pressure."));
