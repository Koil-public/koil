...
public final class LocalRichAttachmentBridge {
    ...
    public static synchronized Text rewrite(Text message) {
        ...
        while (iterator.hasNext()) {
            ...
            if (index >= 0) {
                ...
                String indent = com.spirit.koil.chat.internal.LocalMultilineChatBridge.indentForPrefix(prefix);
                ...
                if (!original.isBlank()) {
                    if (renderAttachmentBelowMessage(attachment)) {
                        ...
                        StringBuilder block = new StringBuilder(prefix)
                                .append(com.spirit.koil.chat.internal.LocalMultilineChatBridge.indentContinuationLines(original, prefix))
                                .append('\n')
    ...
    public static String marker(UUID attachmentId, String linePrefix) {
        ...
        int rows = reservedRows(attachmentId);
    ...
    public static int reservedRows(UUID attachmentId) {
        RichChatAttachment attachment = ATTACHMENTS.get(attachmentId);
        if (attachment != null && attachment.type() == RichChatAttachmentType.AUDIO) {
            return 5;
        }
        if (attachment != null && !(attachment.type() == RichChatAttachmentType.IMAGE || attachment.type() == RichChatAttachmentType.GIF || attachment.type() == RichChatAttachmentType.VIDEO)) {
            return 2;
        }
        if (attachment == null) {
            return reservedRows();
        }
        return RichChatAttachmentRenderer.reservedMarkerRows(attachment);
    }
